...
public class LinearSearchStrategy implements SearchStrategy{
	...
	public int find(Document src, String target, int start, int end,
		...
		if(start < 0){
			EditorException.fail("BaseDocument.find: Invalid start position");
			start = 0;
		}
		if(end > src.length()){
			EditorException.fail("BaseDocument.find: Invalid end position");
			end = src.length();
		}
	...
	public int findBackwards(Document src, String target, int start, int end,
		...
		if(start >= src.length()){
			EditorException.fail("Invalid start position given to BaseDocument.find");
			start = src.length() - 1;
		}
		if(end < -1){
			EditorException.fail("Invalid end position given to BaseDocument.find");
			end = -1;
		}
	...
	protected boolean isSandwichedByWhitespace(Document src,
		...
		boolean startWithWhitespace = (start == 0)
				? true
				: CommonLanguage.isWhitespace(src.charAt(start - 1));

		int end = start + length;
		boolean endWithWhitespace = (end == src.length())
				? true
				:  CommonLanguage.isWhitespace(src.charAt(end));
